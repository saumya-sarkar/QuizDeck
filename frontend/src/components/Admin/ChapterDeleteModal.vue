<template>
  ...
  <div class="modal fade" id="chapterDeleteModal" tabindex="-1" aria-labelledby="chapterDeleteModalLabel" aria-hidden="true" ref="modal">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="chapterDeleteModalLabel">
            Deleting Chapter
        ...
        <div class="modal-body">
          <p class="p-text" v-show="!message">Are you sure you want to delete this chapter {{ name }}?</p>
        ...
        <div class="modal-footer" v-show="!message">
          ...
          <button type="button" class="btn btn-danger" @click="deleteChapter" :disabled="isDeleting">
...
export default {
  ...
  methods: {
    show(chapterToDelete) {
      if (chapterToDelete) {
        this.id = chapterToDelete.id;
        this.name = chapterToDelete.name;
      }
    ...
    async deleteChapter() {
      ...
      try {
        ...
        const response = await axios.patch(`${BASE_URL}/chapter/delete`, {
          id: this.id
        }, 
        {
          headers: {
        ...
        if (response.data.code == 200) {
            ...
            setTimeout(() => {
              ...
              this.$emit('chapter-deleted', { id: this.id, name: this.name });
              this.hide();
            }, 3000);
        }
        ...
        console.error(err);
        this.error = err.response?.data?.error_message || 'Failed to delete chapter. Please try again.';
      } finally {
        this.isDeleting = false;
      }
    }
...
#chapterDeleteModal .btn-close {
  filter: invert(1);
  opacity: 0.8;
}

#chapterDeleteModal .btn-close:hover {
  opacity: 1;
}
...
#chapterDeleteModal .custom-alert-error {
...
#chapterDeleteModal .custom-alert-success {
