<template>
  <div class="chapters-container">
      ...
      <div class="row mb-4">
        <div class="col-12">
          <div class="d-flex justify-content-between align-items-center">
            ...
            <div>
              <button 
                class="btn add-chapter-btn d-flex align-items-center gap-2"
                @click="showChapterCreateModal"
              >
                <font-awesome-icon icon="plus" />
                <span class="d-none d-sm-inline">Add Chapter</span>
              </button>
            </div>
          </div>
        </div>
            ...
            <div class="col-auto">
              <div class="chapter-actions d-flex gap-2">
                ...
                <button 
                  ...
                  @click="showChapterEditModal(chapter)"
                >
                  <font-awesome-icon icon="edit" /> Edit
                </button>
                <button 
                  class="btn btn-outline-danger btn-sm glass-btn"
                  @click="showChapterDeleteModal(chapter)"
    ...
    <!-- Chapter Edit Modal Component -->
    <ChapterEditModal
      ref="chapterEdit"
      @chapter-edited="handleChapterEdited"
    />

    <!-- Chapter Delete Modal Component -->
    <ChapterDeleteModal
      ref="chapterDelete"
...
const ChapterCreateModal = defineAsyncComponent(() =>
  import(/* webpackChunkName: "chapter-create-modal", webpackPrefetch: true */ '@/components/Admin/ChapterCreateModal.vue')
);

const ChapterEditModal = defineAsyncComponent(() =>
  import(/* webpackChunkName: "chapter-edit-modal", webpackPrefetch: true */ '@/components/Admin/ChapterEditModal.vue')
);

const ChapterDeleteModal = defineAsyncComponent(() =>
  import(/* webpackChunkName: "chapter-delete-modal", webpackPrefetch: true */ '@/components/Admin/ChapterDeleteModal.vue')
);
...
export default {
  ...
  methods: {
    ...
    showChapterCreateModal() {
      this.$refs.chapterCreate.show({ subject_id: this.subjectId, subject_name: this.subjectName });
    },

    handleChapterCreated(newChapter) {
      // Show success toast notification
      const toast = useToast();
      toast.success(`Chapter ${newChapter.name} added successfully.`, { theme: 'light' });
      // Add to the list
      if (newChapter) {
    ...
    showChapterEditModal(chapterToEdit) {
      this.$refs.chapterEdit.show({ chapter: chapterToEdit, subject_id: this.subjectId });
    },

    handleChapterEdited(updatedChapter) {
      // Show success toast notification
      const toast = useToast();
      toast.success(`Chapter ${updatedChapter.oldName} updated successfully.`, { theme: 'light' });
      this.fetchChapters();
    },

    showChapterDeleteModal(chapterToDelete) {
      this.$refs.chapterDelete.show(chapterToDelete);
    },

    handleChapterDeleted(deletedChapter) {
      // Show success toast notification
      const toast = useToast();
      toast.success(`Chapter ${deletedChapter.name} deleted successfully.`, { theme: 'light' });
      // Remove the deleted chapter from the list
      this.chapters = this.chapters.filter(chapter => chapter.id !== deletedChapter.id);
    },
